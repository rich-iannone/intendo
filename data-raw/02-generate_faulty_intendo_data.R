...
users_daily_f <- readRDS("data-raw/users_daily.rds")
all_revenue_f <- readRDS("data-raw/all_revenue.rds")
all_sessions_f <- readRDS("data-raw/all_sessions.rds")
user_summary_f <- readRDS("data-raw/user_summary.rds")

random_duplicate_rows <-
  sample(seq_len(nrow(users_daily_f)), size = 124)
...
empty_rows <-
  sample(seq_len(nrow(all_sessions_f)), size = 5)
