...
all_revenue_dd <- function() {
  ...
  pointblank::create_informant(
    ...
    pointblank::info_columns(
      ...
      is corrected with store data. The incidence of this happening is
      generally quite low however."
    ) %>%
    pointblank::info_columns(
      columns = "item_type",
      info = "Either `\"iap\"` or `\"ad\"` which stand for in-app purchase (or
      IAP) and ad view. Each item type has a specific set of `item_name`
      values."
    ) %>%
    pointblank::info_columns(
      columns = "item_name",
...
users_daily_dd <- function() {
  ...
  pointblank::create_informant(
    ...
    pointblank::info_columns(
      ...
      details = "This can be treated as a minimum since events such as crashes
      or app interruptions (e.g., phone calls) don't get captured in the
    ...
    pointblank::info_columns(
      ...
      players will serve to increase the urgency in developing additional
      content for the level designers (making this an important metric)."
    ) %>%
    pointblank::info_columns(
      columns = "in_ftue",
    ...
    pointblank::info_columns(
      ...
      of whales (whales are noted to have daily spends, usually with
      higher-priced IAPs)."
    ) %>%
    pointblank::info_columns(
      columns = "n_ads_total",
      ...
      some regions have high frequencies of interstitial ad placements and
      players designated as grinders are shown substantially more ads."
    ) %>%
    pointblank::info_columns(
    ...
    pointblank::info_columns(
      ...
      info = "This is simply the sum of `rev_iap_total` and `rev_ads_total` for
      a player on up to the given date.",
      details = "These values are based on estimates but are generally
      reasonable especially with greater amounts of revenue. The more uncertain
      component is revenue from ad views."
    ) %>%
    pointblank::info_columns(
